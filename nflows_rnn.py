...
flow_rnn = Flow(transform, base_dist, embedding_net=rnn_encoder)

# Define the optimizer
optimizer = optim.Adam(flow_rnn.parameters(), lr=1e-3)
...
for i in range(num_iter):
    ...
    # Compute the loss
    optimizer.zero_grad()
    loss = -flow_rnn.log_prob(inputs=x, context=ctx).mean()
...
for ix in tqdm(idx_list):
    ...
    with torch.inference_mode():
        log_prob = flow_rnn.log_prob(inputs=x, context=ctx).detach().sum().item()
...
plt.xlim((0, 100))
plt.ylim((-10, 60))
plt.savefig("./figs/experiments/rnn/rnn_pdf.png")
plt.show()
...
********************************************************************************
'''
with h5py.File("observation_sequence.h5", 'w') as f:
    f.create_dataset('true_x', data=state_i[:,0])
    f.create_dataset('true_y', data=state_i[:,1])
    f.create_dataset('obs_x', data=obs_i[:,0])
    f.create_dataset('obs_y', data=obs_i[:,1])
...
def generate_sequence_gif(seq_idx):
    ...
    make_gif("./figs/experiments/rnn/tmp/", 
        "./figs/experiments/rnn/seq{:d}.gif".format(seq_idx), delete_frames=True)
...
embedded_context = flow_rnn._embedding_net(context)
n_points = 400
rep_embedded_context = torchutils.repeat_rows(embedded_context.unsqueeze(0), num_reps=n_points)

params = flow_rnn._distribution._compute_params(embedded_context.unsqueeze(0))
...
region1, ex1, ey1 = extract_region(mu, Sigma, p1, flow_rnn, n_points)
region1 = (region1.detach()*pos_std)+pos_mean

region2, ex2, ey2 = extract_region(mu, Sigma, p2, flow_rnn, n_points)
region2 = (region2.detach()*pos_std)+pos_mean

region3, ex3, ey3 = extract_region(mu, Sigma, p3, flow_rnn, n_points)
...
********************************************************************************
'''
with h5py.File("flow_level_sets_rnn.h5", 'w') as f:
